document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.body.classList.contains('birthday-page-body')) {
        ...
        // ===================================================
        // PEMICU HALAMAN RAHASIA (LOGIKA DIPERBAIKI)
        // ===================================================
        const secretTrigger = document.getElementById('secret-trigger');
        if (secretTrigger) {
            // Variabel penghitung SEKARANG DI LUAR event listener agar tidak di-reset
            let heartClickCount = 0; 
            
            secretTrigger.addEventListener('click', () => {
                // let heartClickCount = 0; <-- Baris ini yang salah & sudah dihapus dari sini
                
                heartClickCount++; // Sekarang akan menghitung dengan benar: 1, 2, 3, 4, 5
                
                // Animasi visual setiap kali diklik
                secretTrigger.classList.add('clicked');
                setTimeout(() => {
                    secretTrigger.classList.remove('clicked');
                }, 400);

                // Cek jika hitungan sudah mencapai 5
        ...
        if (canvas) {
            ...
            let particles = [];
            function createParticles(x, y) { for (let i = 0; i < 15; i++) { particles.push({ x: x + Math.random(), y: y + Math.random(), vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.8) * 0.4, life: 60, }); } }
            function drawParticles() { for (let i = particles.length - 1; i >= 0; i--) { const p = particles[i]; p.x += p.vx; p.y += p.vy; p.life--; if (p.life <= 0) { particles.splice(i, 1); } context.fillStyle = `rgba(233, 64, 87, ${p.life / 60})`; context.save(); context.translate(p.x, p.y); context.font = '0.4px sans-serif'; context.fillText('❤️', 0, 0); context.restore(); } }
            ...
            function playerReset() {
                ...
                if (collide(board, player)) {
                    isGameOver = true;
                    if (player.score < 100) { showModal('Gini doang kok kalah?', [ { text: 'Nyerah ah', class: 'secondary', action: closeGame }, { text: 'Coba Lagi', class: 'primary', action: startGame } ]); }
